...
def update_user(i):
    ...
    users[i]['posts'] = post
    show_users()
    button_dodaj_obiekt.config(text='Dodaj', command=lambda:add_user())
...
def show_user_details():
    ...
    label_liczba_postow_szczegoly_obiektu_wartosc.config(text=users[i]['posts'])


root = Tk()

root.title('Mapbook_ss')
...
button_poka_szczegoly=Button(ramka_lista_obiektow,text='Pokaż szczegoly', command=show_user_details)
button_poka_szczegoly.grid(row=3, column=0)


button_usun_obiekt=Button(ramka_lista_obiektow,text='Usun obiekt', command=remove_user)
...
label_szczegoly_obiektow=Label(ramka_szczegoly_obiektow, text='Szczególy użytkownika')
...
label_liczba_postow_szczegoly_obiektu=Label(ramka_szczegoly_obiektow, text='Postów')
label_liczba_postow_szczegoly_obiektu.grid(row=1, column=6)

label_liczba_postow_szczegoly_obiektu_wartosc=Label(ramka_szczegoly_obiektow, text='.....')
label_liczba_postow_szczegoly_obiektu_wartosc.grid(row=1, column=7)
